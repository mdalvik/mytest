 ...
 */
package com.weixin.zhongli.view.indicator.view.indicator;
...
import com.weixin.zhongli.view.indicator.view.indicator.slidebar.ScrollBar;
import com.weixin.zhongli.view.indicator.view.viewpager.RecyclingPagerAdapter;
...
public class IndicatorViewPager {
	...
	public void setCurrentItem(int item, boolean anim) {
		...
		// 改变选中图片
		for (int i = 0; i < indicatorView.getAdapter().getCount(); i++){
			TextView tv = (TextView) indicatorView.getItemView(i);
			if (item == i){
				tv.setCompoundDrawablesWithIntrinsicBounds(0, IndicatorActivity.tabIcons_select[i], 0, 0);
			} else {
				tv.setCompoundDrawablesWithIntrinsicBounds(0, IndicatorActivity.tabIcons[i], 0, 0);
			}
		}
	}
