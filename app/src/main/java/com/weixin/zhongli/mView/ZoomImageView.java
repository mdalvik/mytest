...
public class ZoomImageView extends ImageView {
	...
	private boolean canRotate = true; // 是否启旋转图片功
	private boolean canDoubleClick = true; // 是否启双击缩放功
	...
	private boolean keyUpDown=true;
	private long now;
					...
					public void run() {
						try {
							...
							if (System.currentTimeMillis() - lastClickTime < 300)
								((Activity)context).finish();// 此处即为OnClick处理对应部分,只需在此处添中处理代码即可.
							keyUpDown=true;
						} catch (InterruptedException e) {
//							L.getStandardErrorMsg("图片缩放控件", " 移动matrix", "com.dtds.tsh.op.view.ZoomImageView.fixTranslation()", e.getMessage(), "");
						}
					}
				}.start();
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			break;
		// 副点按下 ：如果间距大10，记录此时matrix，以及两点的坐标、图片状
